...
function makeDir() {
  ...
  const dirTmp = path.join(os.homedir(), "Documents", "DPSoftware", "tmp");
  if (!fs.existsSync(dir)) { console.log(dir); fs.mkdirSync(dir); }
  if (!fs.existsSync(dirTmp)) { console.log(dirTmp); fs.mkdirSync(dirTmp); }
  const dirNucleus = path.join(os.homedir(), "Documents", "DPSoftware", "Detect Nucleus");
  const dirCytoplasm = path.join(os.homedir(), "Documents", "DPSoftware", "Detect Cytoplasm");
  if (!fs.existsSync(dirNucleus)) { console.log(dirNucleus); fs.mkdirSync(dirNucleus); }
  if (!fs.existsSync(dirCytoplasm)) { console.log(dirCytoplasm); fs.mkdirSync(dirCytoplasm); }
}
...
function loadProject() {
  ...
  let file_paths = store.get(project_name) || null;
  let parsed_file_paths = JSON.parse(file_paths);
  const dirNucleus = path.join(os.homedir(), "Documents", "DPSoftware", project_name, "Detect Nucleus");
  const dirCytoplasm = path.join(os.homedir(), "Documents", "DPSoftware", project_name, "Detect Cytoplasm");
  if (!fs.existsSync(dirNucleus)) { console.log(dirNucleus); fs.mkdirSync(dirNucleus); }
  if (!fs.existsSync(dirCytoplasm)) { console.log(dirCytoplasm); fs.mkdirSync(dirCytoplasm); }
  mainWindow.webContents.send("send_title_and_open_files",project_name, parsed_file_paths);
}
...
ipcMain.on(
  ...
  (event, imageArray, imageTitle, dimensions, project_name) => {
    const dir = path.join(os.homedir(), "Documents", "DPSoftware", "tmp", "tmpLastChannel.json");
...
function nucleiDetect(fileName, width, height, lastChannelPath, projectName) {
  ...
  const pathForTMPchannel = path.join(os.homedir(), "Documents", "DPSoftware", "tmp", "tmp.jpg");
...
ipcMain.on(
  "cytoplasm_info",
  (event, project_name) => {
    const dirToProject = path.join(os.homedir(), "Documents", "DPSoftware", project_name);
