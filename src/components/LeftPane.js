...
export default class LeftPane extends React.Component {
  ...
  render() {
    return (<div style={{width: '400px', height: '100%', backgroundColor: '#F3F3F3', display: 'flex'}}>
      <div style={{width: '44px', backgroundColor: '#1B1B1B', display: 'inline-flex', flexDirection: 'column', alignItems: 'center'}}>
        <ProjectButton
          style={{fill: (this.state.selectedPane === LeftPanes.Project ? '#FFFFFF' : '#808080'), padding: '12px 0px 12px 0px'}}
          onClick={() => {this.setState({selectedPane: LeftPanes.Project})}}
        />
        <ImageButton
          style={{fill: (this.state.selectedPane === LeftPanes.Image ? '#FFFFFF' : '#808080'), padding: '12px 0px 12px 0px'}}
          onClick={() => {this.setState({selectedPane: LeftPanes.Image})}}
        />
        <AnnotationButton 
          style={{fill: (this.state.selectedPane === LeftPanes.Annotation ? '#FFFFFF' : '#808080'), padding: '12px 0px 12px 0px'}}
          onClick={() => {this.setState({selectedPane: LeftPanes.Annotation})}}
        />
      </div>
      <this.paneRender/>
    </div>)
